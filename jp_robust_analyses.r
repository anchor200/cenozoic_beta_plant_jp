...
pairwise_beta <- function(lat, ana=for_ana){
    ...
    if (nrow(as.matrix(select(ana, contains(lat)))) >= 3  ){
        res <- try(beta.pair(t(as.matrix(select(ana, contains(lat))))), silent = FALSE) 
...
removal_iter <- function(removal=0, iter=10, CUT=10, clim=FALSE){
    ...
    for(i in 1:iter){
        ...
        temp2$geo_peri <- factor(temp$coefs$geo_peri, levels=c('Eocene', 'Oligocene', 'Miocene', 'Pliocene', 'Pleistocene', 'LastGlacial', 'Holocene', 'Present'))
...
table_resh <- function(result, CUT, removal, clim=FALSE){
    ...
    regTable <- regTable[, colnames(regTable) != "Eocene"]
    if(clim){
        clim = "clim"
    }else{
        clim=""
...
test_dicay_coef_25 <- removal_iter(removal=0.25, iter=5)

## Figure A6(a)
subset(test_dicay_coef_25, test_dicay_coef_25$predictor=="sp_dist" )%>%
...
subset(test_dicay_coef_25_clim, test_dicay_coef_25_clim$predictor=="clim_dist" )%>%
